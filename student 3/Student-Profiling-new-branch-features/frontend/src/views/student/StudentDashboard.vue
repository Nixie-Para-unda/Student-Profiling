...
// State variables mapped directly to backend expectations
const stats = ref([])
const profileIncomplete = ref(false)
const studentProfile = ref({ gwa: '0.00' })
...
onMounted(async () => {
  try {
    ...
    // Assign backend data to our refs (falling back to empty arrays if undefined)
    studentProfile.value = { ...s }
    studentActivities.value = s.activities || []
    studentAwards.value = s.awards || []
    studentViolations.value = s.violations || []
    todaySchedule.value = s.today_schedule || []
    studentChartData.value = s.chart_data || []
    
    // Build the stats strip using the dynamic data
    stats.value = buildStats(s)
  } catch (err) {
    console.error('Failed to fetch student profile:', err)
    stats.value = buildStats({}) // Fallback for stats if request fails
  }
})
