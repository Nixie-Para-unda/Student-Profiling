    ...
    <div class="hero-banner">
      ...
      <div class="hero-body">
        ...
        <div class="hero-right">
          <div class="hero-stat-card"><span class="hsc-label">This Week</span><span class="hsc-value">{{ activeViolationsCount + pendingApprovalsCount }}</span><span class="hsc-sub">Activities logged</span></div>
          <div class="hero-stat-card accent"><span class="hsc-label">Pending</span><span class="hsc-value">{{ pendingApprovalsCount }}</span><span class="hsc-sub">Awaiting review</span></div>
        </div>
      </div>
    </div>
      ...
      >
        <div class="stat-top">
          ...
          <div class="stat-icon" :style="{ background: stat.iconBg, color: stat.iconColor }">
            <svg viewBox="0 0 18 18" fill="none" v-html="stat.iconPath"></svg>
          </div>
        </div>
    ...
    <div class="bottom-grid">
      ...
      <div class="card">
        ...
        <div class="student-list">
          ...
          <div v-if="topStudents.length === 0" class="empty-state-msg">No top performing students recorded yet.</div>
        </div>
      </div>
      ...
      <div class="card">
        ...
        <div class="violation-list">
          ...
          <div v-if="deanViolations.length === 0" class="empty-state-msg">No active student violations.</div>
        </div>
        <div class="violation-alert" v-if="activeViolationsCount > 2">
          ...
          <button @click="$router.push('/violations')">Review Now →</button>
        </div>
      </div>
    </div>
  </div>
...
const deanViolations = ref([])

const stats = ref([
  { label: 'Total Students', value: '0', delta: 'Real-time sync', deltaClass: 'positive', fill: '0%', iconBg: '#fff5ef', iconColor: '#FF6B1A', route: '/students', iconPath: '<path d="M9 8a3 3 0 100-6 3 3 0 000 6zM2 16a7 7 0 0114 0" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>' },
  { label: 'Total Faculty', value: '0', delta: 'Active members', deltaClass: 'positive', fill: '0%', iconBg: '#eff6ff', iconColor: '#3b82f6', route: '/faculty', iconPath: '<rect x="2" y="2" width="14" height="14" rx="2" stroke="currentColor" stroke-width="1.4"/><path d="M6 6h1m-1 3h1m4-3h1m-1 3h1M6 13v-3a1 1 0 011-1h4a1 1 0 011-1v3" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>' },
  { label: 'Dept. Avg GWA', value: 'N/A', delta: 'Target: 1.75', deltaClass: 'warning', fill: '0%', iconBg: '#f5f3ff', iconColor: '#8b5cf6', iconPath: '<path d="M2 13l3-5 3 3 3-4 5 6H2z" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>' },
  { label: 'With Violations', value: '0', delta: 'Active cases', deltaClass: 'negative', fill: '0%', iconBg: '#fff1f2', iconColor: '#ef4444', iconPath: '<path d="M9 5v4M9 11.5v.5M2.5 14h13a1 1 0 00.87-1.5L10 2.5a1 1 0 00-1.74 0L2.5 12.5A1 1 0 002.5 14z" stroke="currentColor" stroke-width="1.4" stroke-linecap="round"/>' },
  { label: 'Awards Logged', value: '0', delta: 'Recognitions', deltaClass: 'positive', fill: '0%', iconBg: '#fffbeb', iconColor: '#f59e0b', iconPath: '<path d="M9 1.5l1.6 4.8H16l-4.2 3.1 1.6 4.9L9 11.1l-4.4 3.2 1.6-4.9L2 7.3h5.4L9 1.5z" stroke="currentColor" stroke-width="1.4" stroke-linecap="round" stroke-linejoin="round"/>' }
])
...
onMounted(async () => {
  try {
    ...
    if (data.chart_data) chartData.value = data.chart_data
    topStudents.value = data.top_students || []
    deanViolations.value = data.recent_violations || []
    pendingApprovalsCount.value = data.pending_approvals || 0
    activeViolationsCount.value = data.active_violations || 0
    ...
    stats.value = [
      { 
        ...
        deltaClass: 'positive', 
        fill: '100%', 
        iconBg: '#fff5ef', iconColor: '#FF6B1A', route: '/students', 
      ...
      { 
        ...
        deltaClass: 'positive', 
        fill: '100%', 
        iconBg: '#eff6ff', iconColor: '#3b82f6', route: '/faculty', 
      ...
      { 
        ...
        // FIXED: Maps 1.0 -> 100% and 5.0 -> 0%
        fill: avgGwa > 0 ? `${Math.max(0, Math.min(100, ((5.0 - avgGwa) / 4.0) * 100))}%` : '0%', 
      ...
      { 
        ...
        delta: 'Active cases', 
        deltaClass: 'negative', 
        fill: totalStudents > 0 ? (Math.min(100, (activeViolations / totalStudents) * 100) + '%') : '0%', 
