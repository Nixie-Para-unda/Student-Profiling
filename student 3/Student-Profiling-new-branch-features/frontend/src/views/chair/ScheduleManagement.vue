            ...
            <div v-if="form.section_id">
              <div class="form-group">
                ...
                <select v-model="form.course_id">
                  ...
                  <option v-for="c in filteredModalCourses" :key="c.id" :value="c.id">[{{ c.course_code }}] {{ c.course_name }}</option>
                </select>
                <p class="form-hint" v-if="filteredModalCourses.length === 0">No courses found matching your search.</p>
              </div>
...
const fetchSections = async () => {
  try {
    // Fetch sections
    const resSections = await axios.get('/sections')
    sections.value = resSections.data
    
    // NEW: Fetch all programs directly to ensure BSIS is included
    const resPrograms = await axios.get('/programs')
    programs.value = resPrograms.data
  } catch (err) {
    console.error('Failed to fetch sections or programs:', err)
  }
}
...
// Update resetForm to clear the search:
const resetForm = () => {
  form.value = {
    section_id: '',
    course_id: '',
  ...
  modalCourseSearch.value = '' // <-- Add this
}
...
const modalCourseSearch = ref('')

// Add this computed property:
const filteredModalCourses = computed(() => {
  if (!modalCourseSearch.value) return curriculumCourses.value
  
  const query = modalCourseSearch.value.toLowerCase()
  return curriculumCourses.value.filter(c => 
...
.schedule-page { display: flex; flex-direction: column; gap: 24px; font-family: 'Outfit', sans-serif; }
