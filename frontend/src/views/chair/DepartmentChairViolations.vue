<template>
  <div class="page">
    ...
    <!-- VIOLATION DETAILS & ACTION MODAL -->
...
const miniStats = computed(() => [
  { label: 'Total Cases', value: violations.value.length, color: '#FF6B1A' },
  { label: 'Pending Review', value: violations.value.filter(v => v.status === 'Pending').length, color: '#f59e0b' },
  { label: 'Under Review', value: violations.value.filter(v => v.status === 'Under Review').length, color: '#3b82f6' },
  { label: 'Action Taken', value: violations.value.filter(v => ['Resolved', 'Sanctioned'].includes(v.status)).length, color: '#10b981' }
])
